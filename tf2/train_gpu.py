...
def get_model_fn(n_token, cutoffs):
  def model_fn(inp, tgt, mems, is_training):
    inp = tf.transpose(a=inp, perm=[1, 0])
    tgt = tf.transpose(a=tgt, perm=[1, 0])
    ...
    if is_training:
      ...
      grads = tf.gradients(ys=loss, xs=all_vars)
...
def evaluate(n_token, cutoffs, ps_device):
  ...
  with tf.compat.v1.Session(config=tf.compat.v1.ConfigProto(allow_soft_placement=True)) as sess:
    ...
    fetches = [loss, tower_new_mems, tf.size(input=label_feed)]
