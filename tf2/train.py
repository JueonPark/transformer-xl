...
def metric_fn(loss):
  ...
  perplexity = tf.exp(tf.reduce_mean(input_tensor=loss))
  bpc = tf.reduce_mean(input_tensor=loss) / tf.constant(math.log(2))
  return {
...
def get_model_fn(n_token, cutoffs, train_bin_sizes, eval_bin_sizes):
  def model_fn(features, labels, mode, params):
    ...
    mems = params["cache"]
    inp = tf.transpose(a=features["inputs"], perm=[1, 0])
    tgt = tf.transpose(a=features["labels"], perm=[1, 0])

    bin_sizes = train_bin_sizes if is_training else eval_bin_sizes
    if bin_sizes:
      inp_perms = [tf.transpose(a=features["inp_mask"], perm=[1, 0])]
      tgt_perms = [tf.transpose(a=features["tgt_mask"], perm=[1, 0])]

      head_tgt = tf.transpose(a=features["head_labels"], perm=[1, 0])

      for b in range(len(bin_sizes)):
        inp_perm = tf.transpose(a=features["inp_perm_{}".format(b)], perm=[1, 0, 2])
        tgt_perm = tf.transpose(a=features["tgt_perm_{}".format(b)], perm=[1, 0, 2])
